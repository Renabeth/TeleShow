...
with open(json_path1,'r') as file:  #Opens and parses file based on path given from the variables created above.
...
def watchmode_search(tmdb_id, type): #Function takes tmdb id and type (movie or tv)
    ...
    if type == 'movie': 
        ...
        if response.status_code == 200: #Watchmode code 200 means request was successful
            ...
            if results: #Iterates through the results given and gets data under ID key from results
...
def calculate_relevance(item, query): #Makes sure that items with the query in the name get precidence
...
@app.route('/search',methods=['GET']) #What happens when someone visits the /search endpoint of this Flask API
def search():
    query = request.args.get('query') #Gets query variable passed from React 
    filter_type = request.args.get('filter_type',"all") #Gets filter choice passed from React 
    ...
    if not query: 
        ...
        try:    
            ...
            if filter_type in ['all','movie']: #Filters results
                ...
                movie_search.movie(query = query) #Searchs tmdb API for movies that have query in there names.
                movie_results = movie_search.results

                for movie in movie_results: #Iterates through results and completes the incomplete poster path given in respoosne 
                    poster_path = movie.get('poster_path')
                    if poster_path:
                        movie['poster_url'] = f"{image_url}{poster_path}"
                    movie['media_type'] ='movie' #assigns type key:value pairs to all results
                
                movie_results = sorted(movie_results, key=lambda x: x.get('popularity', 0), reverse=True) #Sorts all results in order of popularity via popularity key.
            ...
            if filter_type in ['all','tv']:
                ...
                tv_results = sorted(tv_results, key=lambda x: calculate_relevance(x,query), reverse=True)

            
            return jsonify({'tmdb_movie': movie_results or [], 'tmdb_tv': tv_results or []}) #Returns both Movie and TV results or empty array if no results are found.

        except Exception as e:
            return jsonify({'error':str(e)}), 500
        
@app.route('/search/details',methods=['GET']) #Search/details enpoint (this handles the extra data given when you click a result)
def details():
    ...
    if not item_id or not item_type:
        ...
        try:
            if item_type =='movie':
                media = tmdb.Movies(item_id) #Searches using the media id instead of name for a 1to1 result across tmdb and watchmode
                watchmode_data = watchmode_search(item_id, item_type) #Passes the tmdb_id and type to watchmode search function
            elif item_type == 'tv':
